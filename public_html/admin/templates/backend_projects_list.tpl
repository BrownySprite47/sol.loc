{if $bProjectsEdit}<div class="sub_links">
...
<td>
<div class="wrap_input inline_input">
<input id="project_done_type_1" type="radio" name="project_done_type_id" value="1"{if $aSearch.project_done_type_id eq "1"} checked="checked"{/if} /> <label for="project_done_type_1">все</label>
</div>
<div class="wrap_input inline_input">
<input id="project_done_type_2" type="radio" name="project_done_type_id" value="2"{if $aSearch.project_done_type_id eq "2"} checked="checked"{/if} /> <label for="project_done_type_2">заполенные анкеты</label>
</div>
<div class="wrap_input inline_input">
<input id="project_done_type_3" type="radio" name="project_done_type_id" value="3"{if $aSearch.project_done_type_id eq "3"} checked="checked"{/if} /> <label for="project_done_type_3">незаполненные анкеты</label>
</div>
</td>
</tr>
<tr{cycle name="content_data" values=' class="odd",'}>
...
<div class="wrap_input inline_input">
<input id="project_enabled_type_2" type="radio" name="project_enabled_type_id" value="2"{if $aSearch.project_enabled_type_id eq "2"} checked="checked"{/if} /> <label for="project_enabled_type_2">актуальные</label>
</div>
<div class="wrap_input inline_input">
<input id="project_enabled_type_3" type="radio" name="project_enabled_type_id" value="3"{if $aSearch.project_enabled_type_id eq "3"} checked="checked"{/if} /> <label for="project_enabled_type_3">неактуальные</label>
</div>
</td>
</tr>
</tbody>
...
{if $bProjectsEdit}<th class="small"></th>{/if}
</tr>
</thead>
<tbody>
{foreach from=$aContentList item=item}
<tr class="{cycle name="content_list" values='odd,'}{if $item.project_enabled eq 0} disable{/if}">
<td>{$item.project_id}</td>
<td><a target="_blank" href="{#PROJECT_BACKEND_URL#}index.php?module_name=projects&action_name=view{if !$bProjectsEdit}_without_edit{/if}&content_id={$item.project_id}">{$item.project_name_small_show}</a></td>
...
<td>{if isset($item.leaders) and !empty($item.leaders)}{foreach from=$item.leaders item=i}{if $i.leader_id eq 0}{$i.leader_name}{else}<a target="_blank" href="{#PROJECT_BACKEND_URL#}index.php?module_name=leaders&action_name=view&content_id={$i.leader_id}">{$i.leader_name}</a>{/if}<br/>{/foreach}{/if}</td>
<td>{$item.project_interview_backend_user_name}</td>
<td>{if $item.project_site_enabled eq 1}<a href="{$item.project_site}" target="_blank">URL</a>{else}{$item.project_site}{/if}</td>
<td>{if $item.project_done eq 1}+{else}-{/if}</td>
{if $bProjectsEdit}<td><a target="_blank" href="{#PROJECT_BACKEND_URL#}index.php?module_name=projects&action_name=view_without_edit&content_id={$item.project_id}"><img src="{#PROJECT_BACKEND_URL#}images/view.png" alt="Просмотр" width="15" /></a></td>{/if}
</tr>
{/foreach}
</tbody>
</table>
