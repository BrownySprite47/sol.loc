...
<td colspan="2"><input type="text" name="project_create_date" value="{if isset($aContentData)}{$aContentData.project_create_date}{else}{if isset($aContentDataDefault)}{$aContentDataDefault.project_create_date}{else}{$sProjectCreateDateRecommend}{/if}{/if}" class="small{if isset($aContentDataErrors.project_create_date)} error{/if}" id="project_create_date" /> рекомендуемая дата: <a href="#" onclick="$('#project_create_date').val('{$sProjectCreateDateRecommend}'); return false;">{$sProjectCreateDateRecommend}</a><p class="error_text">{if isset($aContentDataErrors.project_create_date)}{$aContentDataErrors.project_create_date}{/if}</p></td>
</tr>
<tr{cycle name="content_data_01" values=' class="odd",'}>
...
<td style="width: 20%;"><a href="{#PROJECT_BACKEND_URL#}index.php?module_name=leaders&action_name=view&content_id={$aContentData.leader_id}" target="_blank">{$aContentData.leader_surname}{if $aContentData.leader_name ne ""} {$aContentData.leader_name}{if $aContentData.leader_patronymic ne ""} {$aContentData.leader_patronymic}{/if}{/if}{if $aContentData.project_name_1 ne ""} ({$aContentData.project_name_1}){/if}</a></td>
{/if}
...
<td>Статус анкеты</td>
<td colspan="2">
<input id="project_enabled" type="checkbox" name="project_enabled"{if !isset($aContentData) or $aContentData.project_enabled eq 1} checked{/if} /> <label for="project_enabled">анкета актуальна</label>
<input id="project_done" type="checkbox" name="project_done"{if isset($aContentData) and $aContentData.project_done eq 1} checked{/if} /> <label for="project_done">анкета заполнена</label>
</td>
</tr>
</tbody>
</table>
{if isset($aContentData.project_id) and $bProjectsEdit}
<div class="options_add" for="block_02" id="header-fixed-fio">{if isset($aContentData)}{$aContentData.project_name}{/if}</div>
...
<div class="options_add" for="leaders">Лидеры проекта ({if isset($aLeaders)}{count($aLeaders)}{else}0{/if})</div>
...
{if isset($aContentData.project_id, $aLeaders) and $bLeaderProjectDeleteEnabled}<th class="small"></th>{/if}
</tr>
...
{else}
<td colspan="4"><a href="{#PROJECT_BACKEND_URL#}index.php?module_name=leaders&action_name=view&content_id={$item.leader_id}" target="_blank">{$item.leader_surname}{if $item.leader_name ne ""} {$item.leader_name}{if $item.leader_patronymic ne ""} {$item.leader_patronymic}{/if}{/if}{if $item.project_name ne ""} ({$item.project_name}){/if}</a></td>
{/if}
...
{if isset($aContentData.project_id, $aLeaders) and $bLeaderProjectDeleteEnabled}<td></td>{/if}
</tr>
{/section}
</table>
...
<div class="options_add" for="filials">Филиалы ({if isset($aFilials)}{count($aFilials)}{else}0{/if})</div>
...
{if isset($aContentData.project_id, $aFilials) and $bFilialDeleteEnabled}<th class="small"></th>{/if}
</tr>
{assign var="iFilialOrderMax" value="0"}
{if isset($aFilials)}
{foreach from=$aFilials item=item}
{if $iFilialOrderMax lt $item.filial_order}{assign var="iFilialOrderMax" value="`$item.filial_order`"}{/if}
<tr{cycle name="filials" values=' class="odd",'}>
<td class="small">{if isset($aCities)}<select name="city_id_old[{$item.filial_id}]"><option></option>{foreach from=$aCities item=i}<option value="{$i.city_id}"{if $item.city_id eq $i.city_id} selected{/if}>{$i.city_name}</option>{/foreach}</select>{/if}</td>
<td><input type="text" name="filial_city_name_old[{$item.filial_id}]" placeholder="другой город" value="{$item.filial_city_name}" /></td>
<td><textarea name="filial_address_old[{$item.filial_id}]">{$item.filial_address}</textarea></td>
<td><textarea name="filial_comment_old[{$item.filial_id}]">{$item.filial_comment}</textarea></td>
<td><input type="text" name="filial_order_old[{$item.filial_id}]" class="small" value="{$item.filial_order}" /></td>
{if isset($aContentData.project_id) and $bFilialDeleteEnabled}<td><a href="{#PROJECT_BACKEND_URL#}index.php?module_name=projects&action_name=filial_delete&content_id={$item.filial_id}" onclick="return confirm('Вы уверены, что хотите удалить?');"><img src="{#PROJECT_BACKEND_URL#}images/delete.png" alt="Удалить" /></a></td>{/if}
</tr>
{/foreach}
{/if}
{section name=for loop=3}
{assign var="iFilialOrderMax" value="`$iFilialOrderMax+1`"}
<tr{cycle name="filials" values=' class="odd",'}>
<td class="small">{if isset($aCities)}<select name="city_id_new[{$smarty.section.for.iteration}]"><option></option>{foreach from=$aCities item=item}<option value="{$item.city_id}">{$item.city_name}</option>{/foreach}</select>{/if}</td>
<td><input type="text" name="filial_city_name_new[{$smarty.section.for.iteration}]" placeholder="другой город" /></td>
<td><textarea name="filial_address_new[{$smarty.section.for.iteration}]"></textarea></td>
<td><textarea name="filial_comment_new[{$smarty.section.for.iteration}]"></textarea></td>
<td><input type="text" name="filial_order_new[{$smarty.section.for.iteration}]" class="small" value="{$iFilialOrderMax}" /></td>
{if isset($aContentData.project_id, $aFilials) and $bFilialDeleteEnabled}<td></td>{/if}
</tr>
{/section}
</table>
