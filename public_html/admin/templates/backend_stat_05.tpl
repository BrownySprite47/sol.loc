...
{foreach from=$aBackendUsers item=item}<th colspan="2"><strong>{$item}</strong></th>{/foreach}
</tr>
{foreach from=$aContentData item=item key=key}
<tr{cycle name="content_list" values=' class="odd",'}>
{if $key eq "all" or $key eq "all_0"}
<td><strong>Всего{if $key eq "all_0"} назначено{/if}</strong></td>
{foreach from=$aBackendUsers item=i key=k}
{if isset($item[$k])}
<td><strong>{$item[$k]["leaders_done_count"]}</strong></td>
...
{/foreach}
{else}
<td>{if $key eq "-"}интервью не назначено{else}{if $key eq "--"}-{else}{$key}{/if}{/if}</td>
{foreach from=$aBackendUsers item=i key=k}
{if $k eq "all"}
{if isset($item[$k])}
<td><strong>{$item[$k]["leaders_done_count"]}</strong></td>
...
{else}
{if isset($item[$k])}
<td>{$item[$k]["leaders_done_count"]}</td>
