...
<tr{cycle name="content_data" values=' class="odd",'}>
<td>Регион *</td>
<td><select name="region_id"{if isset($aContentDataErrors.region_id)} class="error"{/if}>{if !isset($aContentData) or $aContentData.region_id eq ""}<option></option>{/if}{foreach from=$aRegions item=item}<option value="{$item.region_id}"{if isset($aContentData) and $aContentData.region_id eq $item.region_id} selected{/if}>{$item.region_name}</option>{/foreach}</select><p class="error_text">{if isset($aContentDataErrors.region_id)}{$aContentDataErrors.region_id}{/if}</p></td>
</tr>
{/if}
<tr{cycle name="content_data" values=' class="odd",'}>
<td>Сортировка *</td>
