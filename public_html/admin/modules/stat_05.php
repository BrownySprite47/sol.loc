...
if(isset($_GET["date_from"]) and bIsDate($_GET["date_from"]))
{
  $aSearch["date_from"] = $_GET["date_from"];
}

if(isset($_GET["date_to"]) and bIsDate($_GET["date_to"]))
{
...
$aWhereLeaders[] = "l.leader_enabled = 1";

$aStatTypes = array();
$aStatTypes[1] = array("time_prefix" => "", "time_postfix" =>"", "period_name_show" => "COALESCE(l.leader_interview_date, '-')");
$aStatTypes[2] = array("time_prefix" => "YEARWEEK(", "time_postfix" =>", -1)", "period_name_show" => "COALESCE(CONCAT(DATE_FORMAT(STR_TO_DATE(CONCAT(YEARWEEK(l.leader_interview_date, -1), 'Monday'), '%x%v%W'), '%Y-%m-%d'), ' - ', DATE_FORMAT(STR_TO_DATE(CONCAT(YEARWEEK(l.leader_interview_date, -1), 'Sunday'), '%x%v%W'), '%Y-%m-%d')), '-')");
$aStatTypes[3] = array("time_prefix" => "DATE_FORMAT(", "time_postfix" =>", '%Y-%m')", "period_name_show" => "COALESCE(DATE_FORMAT(l.leader_interview_date, '%Y-%m'), '-')");
$aStatTypes[4] = array("time_prefix" => "YEAR(", "time_postfix" =>")", "period_name_show" => "COALESCE(YEAR(l.leader_interview_date), '-')");
$aStatTypes[5] = array("time_prefix" => "IF(", "time_postfix" =>" IS NULL, '-', 'all')", "period_name_show" => "IF(l.leader_interview_date IS NULL, '-', '--')");
...
$sSql = "SELECT
  ...
  SUM(l.leader_done) AS leaders_done_count
FROM
...
{
  $aContentTotal = array();
  $aContentTotal["all"] = array("leaders_count" => 0, "leaders_done_count" => 0);
  $aContentData = array();
  $aBUIds = array();

  while($aRow = $oResult->fetch_assoc())
  {
    if($aRow["period_name_show"] === "-" and !isset($aContentData["all_0"]) and !empty($aContentTotal))
    {
      $aContentData["all_0"] = $aContentTotal;
    }
    ...
    if(!isset($aContentData[$aRow["period_name_show"]]))
    {      $aContentData[$aRow["period_name_show"]] = array();
      $aContentData[$aRow["period_name_show"]]["all"] = array("leaders_count" => 0, "leaders_done_count" => 0);    }
    $aContentData[$aRow["period_name_show"]][$aRow["backend_user_id"]] = $aRow;
    $aContentData[$aRow["period_name_show"]]["all"] = array("leaders_count" => $aContentData[$aRow["period_name_show"]]["all"]["leaders_count"] + $aRow["leaders_count"], "leaders_done_count" => $aContentData[$aRow["period_name_show"]]["all"]["leaders_done_count"] + $aRow["leaders_done_count"]);

    if(!isset($aContentTotal[$aRow["backend_user_id"]]))
    {      $aContentTotal[$aRow["backend_user_id"]] = array("leaders_count" => 0, "leaders_done_count" => 0);    }
    $aContentTotal[$aRow["backend_user_id"]] = array("leaders_count" =>  $aContentTotal[$aRow["backend_user_id"]]["leaders_count"] + $aRow["leaders_count"], "leaders_done_count" =>  $aContentTotal[$aRow["backend_user_id"]]["leaders_done_count"] + $aRow["leaders_done_count"]);
    $aContentTotal["all"] = array("leaders_count" =>  $aContentTotal["all"]["leaders_count"] + $aRow["leaders_count"], "leaders_done_count" =>  $aContentTotal["all"]["leaders_done_count"] + $aRow["leaders_done_count"]);
  }
  $oResult->close();
  ...
  if(!empty($aContentData))
  {  	$aBUIds["all"] = true;
  	...
  	if($bExcelEnabled)
    {
      ...
      foreach($aBackendUsers as $iBackenUserId => $sBackenUserName)
      {      	$aFields[$iBackenUserId . "_leaders_done"] = $sBackenUserName . " (анкета заполнена)";
      	$aFields[$iBackenUserId . "_leaders"] = $sBackenUserName . " (всего)";      }
      ...
      foreach($aContentData as $sPeriodName => $aData)
      {      	$iColumn = 0;
        ...
        {          if($sPeriodName === "all_0")
          {
            $oPage->setCellValueByColumnAndRow($iColumn, $iRow, "Всего назначено");
          }
          else
          {
            if($sPeriodName === "-")
            ...
            else
            {
              if($sPeriodName === "--")
              {
                $oPage->setCellValueByColumnAndRow($iColumn, $iRow, "-");
              }
              else
              {
        ...
        foreach($aBackendUsers as $iBackenUserId => $sBackenUserName)
        {
          ...
          {          	$oPage->setCellValueByColumnAndRow($iColumn, $iRow, $aData[$iBackenUserId]["leaders_done_count"]);
          	$iColumn++;
