...
$aWhereLeaders = array();
$aWhereLeaders[] = "l.leader_create_date IS NOT NULL";
$aWhereRecommendations = array();
$aWhereRecommendations[] = "TRUE";
$aWhereProjects = array();
$aWhereProjects[] = "p.project_create_date IS NOT NULL";
...
{
  $aWhereLeaders[] = "l.leader_create_date >= '" . $aSearch["date_from"] . "'";
  $aWhereRecommendations[] = "r.recommendation_create_datetime >= '" . $aSearch["date_from"] . "'";
  $aWhereProjects[] = "p.project_create_date >= '" . $aSearch["date_from"] . "'";
}

if($aSearch["date_to"] !== "")
{
  $aWhereLeaders[] = "l.leader_create_date <= '" . $aSearch["date_to"] . "'";
  $aWhereRecommendations[] = "r.recommendation_create_datetime < '" . $aSearch["date_to"] . "' + INTERVAL 1 DAY";
  $aWhereProjects[] = "p.project_create_date <= '" . $aSearch["date_to"] . "'";
}
...
$aWhereProjects[] = "p.project_enabled = 1";
...
if($bExcelEnabled)
{
  ...
  $aFields["leaders_count_4"] = "Количество лидеров ЛИСС (анкета заполнена)";
  $aFields["leaders_count_5"] = "Количество лидеров ЛИСС";
  $aFields["leaders_count_6"] = "Количество лидеров ЛИСС (количество входящих рекомендаций не менее 2-ух)";
  $aFields["projects_count_1"] = "Количество проектов ЛИСС (анкета заполнена)";
  $aFields["projects_count_2"] = "Количество проектов ЛИСС";
...
$sSql = "SELECT
  ...
  SUM(t.leaders_count_6) AS leaders_count_6,
  SUM(t.projects_count_1) AS projects_count_1,
  SUM(t.projects_count_2) AS projects_count_2,
  SUM(t.recommendations_count_1) AS recommendations_count_1,
  SUM(t.recommendations_count_2) AS recommendations_count_2
FROM
  ((SELECT
    " . $aStatTypes[$aSearch["stat_type_id"]]["time_prefix"] . "l.leader_create_date" . $aStatTypes[$aSearch["stat_type_id"]]["time_postfix"] . " AS period_name,
    ...
    SUM(l.leader_done) AS leaders_count_4,
    COUNT(l.leader_id) AS leaders_count_5,
    0 AS leaders_count_6,
    0 AS projects_count_1,
    0 AS projects_count_2,
  ...
  UNION ALL
  (SELECT
    " . $aStatTypes[$aSearch["stat_type_id"]]["time_prefix"] . "p.project_create_date" . $aStatTypes[$aSearch["stat_type_id"]]["time_postfix"] . " AS period_name,
    ...
    0 AS leaders_count_6,
    SUM(p.project_done) AS projects_count_1,
    COUNT(p.project_id) AS projects_count_2,
    0 AS recommendations_count_1,
    0 AS recommendations_count_2
  FROM
  ...
  (SELECT
    ...
    0 AS projects_count_1,
    0 AS projects_count_2,
    SUM(IF(l.leader_id IS NOT NULL AND l.leader_enabled = 1, 1, 0)) AS recommendations_count_1,
    SUM(IF(l.leader_id IS NULL OR l.leader_enabled = 1, 1, 0)) AS recommendations_count_2
  FROM
