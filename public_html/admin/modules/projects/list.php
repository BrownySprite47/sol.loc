...
{
  $aWhere[] = "p.project_create_date >= '" . $aSearch["date_from"] . "'";
}

if($aSearch["date_to"] !== "")
{
  $aWhere[] = "p.project_create_date <= '" . $aSearch["date_to"] . "'";
}
...
if(isset($_GET["project_done_type_id"]) and bIsInt($_GET["project_done_type_id"], 1, 3))
{
  $aSearch["project_done_type_id"] = $_GET["project_done_type_id"];
}
else
{
  $aSearch["project_done_type_id"] = "1";
}

switch($aSearch["project_done_type_id"])
{
  case "1":
  {
  	break;
  ...
  case "2":
  {
  	$aWhere[] = "p.project_done = 1";
  	break;
  }

  case "3":
  {
  	$aWhere[] = "p.project_done = 0";
  	break;
  }
}
...
$sSql = "SELECT
  ...
  COALESCE(IF(ov_11.option_value_small = '', ov_11.option_value, ov_11.option_value_small), '') AS project_option_11,
  COALESCE(IF(ov_12.option_value_small = '', ov_12.option_value, ov_12.option_value_small), '') AS project_option_12,
  ...
  p.project_area,
  COALESCE(GROUP_CONCAT(DISTINCT IF(cov.option_value_id IS NULL, NULL, IF(ov_5.option_value_small = '', ov_5.option_value, ov_5.option_value_small)) ORDER BY ov_5.option_order, ov_5.option_value SEPARATOR ', '), '') AS project_option_5
FROM
  " . DB_PREFIX . "projects AS p
