...
$sProjectCreateDateRecommend = date("Y-m-d");
...
if(isset($_GET["content_id"]) and bIsInt($_GET["content_id"], 1))
{
  ...
  p.project_done,
  SUBSTRING_INDEX(COALESCE(GROUP_CONCAT(COALESCE(IF(p_1.project_name_small = '', p_1.project_name, p_1.project_name_small), lp_1.project_name, '') ORDER BY lp_1.project_order, lp_1.project_name, p_1.project_name, p_1.project_create_datetime, p_1.project_id SEPARATOR '###'), ''), '###', 1) AS project_name_1
FROM
  ...
  LEFT JOIN " . DB_PREFIX . "leaders_projects AS lp_1 ON l.leader_id = lp_1.leader_id
  LEFT JOIN " . DB_PREFIX . "projects AS p_1 ON lp_1.project_id = p_1.project_id
WHERE
...
LIMIT
  ...
  if($oResult = $oDB->query($sSql))
  {
    if($aRow = $oResult->fetch_assoc())
    {
      ...
      $aRow["project_name_1"] = htmlspecialchars($aRow["project_name_1"]);
      ...
      if($aRow["project_interview_date"] !== "" and $aRow["project_interview_date"] < $sProjectCreateDateRecommend)
      {      	$sProjectCreateDateRecommend = $aRow["project_interview_date"];      }

      if(($sProjectCreateDateRecommend . " 00:00:00") > $aRow["project_create_datetime"])
      {
      	$sProjectCreateDateRecommend = substr($aRow["project_create_datetime"], 0, 10);
      }
  ...
  COALESCE(l.leader_create_date, '') AS project_create_date_recommend_1,
  COALESCE(DATE(l.leader_create_datetime), '') AS project_create_date_recommend_2,
  COALESCE(l.leader_interview_date, '') AS project_create_date_recommend_3,
  SUBSTRING_INDEX(COALESCE(GROUP_CONCAT(COALESCE(IF(p_1.project_name_small = '', p_1.project_name, p_1.project_name_small), lp_1.project_name, '') ORDER BY lp_1.project_order, lp_1.project_name, p_1.project_name, p_1.project_create_datetime, p_1.project_id SEPARATOR '###'), ''), '###', 1) AS project_name
FROM
...
ORDER BY
  ...
  l.leader_id";
      if($oLeadersResult = $oDB->query($sSql))
      {
        ...
        {
          $aLPRow["leader_role"] = htmlspecialchars($aLPRow["leader_role"]);
          $aLPRow["leader_surname"] = htmlspecialchars($aLPRow["leader_surname"]);
          $aLPRow["leader_name"] = htmlspecialchars($aLPRow["leader_name"]);
          $aLPRow["leader_patronymic"] = htmlspecialchars($aLPRow["leader_patronymic"]);
          $aLPRow["project_name"] = htmlspecialchars($aLPRow["project_name"]);

          $oSmarty->append("aLeaders", $aLPRow);

          if($aLPRow["project_create_date_recommend_1"] !== "" and $sProjectCreateDateRecommend > $aLPRow["project_create_date_recommend_1"])
          {
          	$sProjectCreateDateRecommend = $aLPRow["project_create_date_recommend_1"];
          }

          if($aLPRow["project_create_date_recommend_2"] !== "" and $sProjectCreateDateRecommend > $aLPRow["project_create_date_recommend_2"])
          {
          	$sProjectCreateDateRecommend = $aLPRow["project_create_date_recommend_2"];
          }

          if($aLPRow["project_create_date_recommend_3"] !== "" and $sProjectCreateDateRecommend > $aLPRow["project_create_date_recommend_3"])
          {
          	$sProjectCreateDateRecommend = $aLPRow["project_create_date_recommend_3"];
          }
        }
...
ORDER BY
  ...
  f.filial_id";
      if($oFilialsResult = $oDB->query($sSql))
      {
        ...
        {
          $aFilialRow["filial_city_name"] = htmlspecialchars($aFilialRow["filial_city_name"]);
          $aFilialRow["filial_address"] = htmlspecialchars($aFilialRow["filial_address"]);
          $aFilialRow["filial_comment"] = htmlspecialchars($aFilialRow["filial_comment"]);

          $oSmarty->append("aFilials", $aFilialRow);
        }
...
if(isset($_GET["leader_id"]) and bIsInt($_GET["leader_id"], 1) and $iContentId === 0)
{  $sSql = "SELECT
  ...
  COALESCE(l.leader_interview_date, DATE(NOW()) AS project_create_date
FROM
...
$oSmarty->assign("sProjectCreateDateRecommend", $sProjectCreateDateRecommend);
...
$oSmarty->assign("bFilialDeleteEnabled", isset($aBackendUserInfo["backend_access_types"]["filial_delete_enabled"]));
$oSmarty->assign("bProjectsEdit", isset($aBackendUserInfo["backend_access_types"]["projects_edit"]));
...
else
{
  if($iContentId === 0)
  {
  	header("Location: " . PROJECT_BACKEND_URL . "index.php?module_name=projects&action_name=list");
  }
  else
  {
  	$oSmarty->assign("sInnerPage", "backend_projects_view_without_edit");
    $oSmarty->display("backend_main.tpl");
  }
}
