<div class="sub_links">
{if isset($aContentData.leader_id)}<a href="{#PROJECT_BACKEND_URL#}index.php?module_name=leaders&action_name=view">создать лидера ЛИСС</a> | {/if}
...
<td colspan="2"><input type="text" name="leader_birth_date" value="{if isset($aContentData)}{$aContentData.leader_birth_date}{/if}" class="small" id="leader_birth_date" /> <input id="leader_birth_date_correct" type="checkbox" name="leader_birth_date_correct"{if isset($aContentData) and $aContentData.leader_birth_date_correct eq 1} checked{/if} /> <label for="leader_birth_date_correct">дата точная</label></td>
</tr>
<tr{cycle name="content_data_02" values=' class="odd",'}>
...
{if isset($aContentData.leader_id)}
<div class="options_add open" for="projects">Проекты ({if isset($aProjects)}{count($aProjects)}{else}0{/if})</div>
...
<td colspan="2"><a href="{#PROJECT_BACKEND_URL#}index.php?module_name=projects&action_name=view&content_id={$item.project_id}" target="_blank">{$item.project_name} ({$item.project_id})</a></td>
{else}
...
{if isset($aContentData.leader_id)}
<div class="options_add open" for="leaders">Рекомендации ({$aContentData.recommendations_to_count} / {if isset($aRecommendations)}{count($aRecommendations)}{else}0{/if})</div>
...
{if isset($aContentData.leader_id) and $bRecommendationDeleteEnabled}<th class="small"></th>{/if}
</tr>
...
<td colspan="2"><a href="{#PROJECT_BACKEND_URL#}index.php?module_name=leaders&action_name=view&content_id={$item.leader_id_to}" target="_blank">{$item.leader_surname}{if $item.leader_name ne ""} {$item.leader_name}{if $item.leader_patronymic ne ""} {$item.leader_patronymic}{/if}{/if}</a></td>
...
{if isset($aContentData.leader_id) and $bRecommendationDeleteEnabled}<td rowspan="6"></td>{/if}
</tr>
<tr{if $iTemp eq 1} class="odd"{/if}>
